...
void main() {
    ...
    if(distance > minFogDistance){
        float a = maxFogDistance - distance;
        float b = maxFogDistance - minFogDistance;
        float ff = (1.0-(a / b));
        
        ff = pow((1.0 / 2.71828),((distance-minFogDistance) * 0.06));
        ff = 1.0 - ff;
        
        fogFactor = clamp(ff, 0.0, 1.0);  
    }
    ...
    //overblending
    fColor = (vec4(totalLighting, 1.0) *(1.0-fogFactor)) + (fogColor * fogFactor);
}`;